...
#mask_b: iteration decoding initial mask, where mask_b is true means mask
    def inpainting(self,image,mask_b,i): #MakGIT inference
        ...
        self.model.eval()
        with torch.no_grad():
            _, z_indices_gt = self.model.encode_to_z(image) #z_indices: masked tokens (b,16*16)
            mask_num = mask_b.sum() #total number of mask token 

            z_indices_predict = z_indices_gt
            mask_bc=mask_b
            ...
            for step in range(self.total_iter):
                ...
                z_indices_predict, mask_bc = self.model.inpainting(z_indices_predict,ratio,mask_bc,mask_num)
...
#TODO3 step1-2: modify the path, MVTM parameters
    parser.add_argument('--load-transformer-ckpt-path', type=str, default='C:/Users/kenny/Downloads/pt/pt/best_model1.pt', help='load ckpt')
    
    #dataset path
    parser.add_argument('--test-maskedimage-path', type=str, default='lab3_dataset/lab3_dataset/masked_image', help='Path to testing image dataset.')
    parser.add_argument('--test-mask-path', type=str, default='lab3_dataset/lab3_dataset/mask64', help='Path to testing mask dataset.')
    #MVTM parameter
    parser.add_argument('--sweet-spot','-s', type=int, default=4, help='sweet spot: the best step in total iteration')
    parser.add_argument('--total-iter','-t', type=int, default=5, help='total step for mask scheduling')
    parser.add_argument('--mask-func', '-m',type=str, default='cosine', help='mask scheduling function')
